...
def setup(args):
    ...
    for folder in ["images", "videos", "metrics", "configs_bk", "buffer_maps", "backup", "point_clouds"]:
...
def main(args):
    ...
    if args.resume_from is not None:
        ...
        if hasattr(trainer, 'save_pointcloud'):
            trainer.save_pointcloud(aabb=dataset.get_aabb().numpy(), save_dir = os.path.join(cfg.log_dir, 'point_clouds', f'points_{0}.ply'))
        logger.info(
    ...
    for step in metric_logger.log_every(all_iters, cfg.logging.print_freq):
        ...
        if step % cfg.logging.vis_freq == 0 and cfg.logging.vis_freq > 0:
            if hasattr(trainer, 'save_pointcloud'):
                trainer.save_pointcloud(aabb=dataset.get_aabb().numpy(), save_dir = os.path.join(cfg.log_dir, 'point_clouds', f'points_{step}.ply'))
        ...
        if hasattr(trainer, "cull_importance"):
            trainer.cull_importance(step=step, dataset=dataset.train_image_set)
        if hasattr(trainer, "finer_decompose"):
            trainer.finer_decompose(step=step)

...
def remove_instance(trainer, remove_id: List[int], model_name = 'Dynamic'):
    trainer.models[model_name].remove_instances(remove_id)
    return trainer

def collect_gaussians_from_ids(trainer, collect_id: List[int], model_name = 'Dynamic'):
    gs_dict = trainer.models[model_name].collect_gaussians_from_ids(collect_id)
    return gs_dict

def replace_instance(trainer, replace_dict: Dict[int, int], new_gaussians_dict:  Dict, model_name = 'Dynamic'):
    trainer.models[model_name].replace_instances(replace_dict, new_gaussians_dict)
    return trainer
...
def main(args):
    ...
    cluster_points_dict = {}
    cluster_color_dict = {}
    for i in range(trainer.models['DynamicNodes'].num_instances):
        ret = trainer.models['DynamicNodes'].export_gaussians_to_ply_instance(i)
        cluster_points_dict[i] = ret['positions'].detach()[:5000]
        cluster_color_dict[i] = ret['colors'].detach()[:5000]
    vis_every_cluster(cluster_points_dict, cluster_color_dict, output_folder=log_dir)

    edit_cfg = OmegaConf.load(os.path.join(log_dir, args.edit_cfg))
    edit_task = edit_cfg['task']

    if edit_task == 'remove':
        trainer = remove_instance(trainer, **edit_cfg.args)
    elif edit_task == 'collect':
        gs_dict = collect_gaussians_from_ids(trainer, **edit_cfg.args)
        for k,v in gs_dict.items():
            save_path = os.path.join(log_dir, "instance_ckpt", f"{k}.pt")
            torch.save(v, save_path)
        return
    elif edit_task == 'replace':
        gs_dict = {}
        for k, v in edit_cfg.args.replace_dict.items():
            gs_dict[v] = torch.load(os.path.join(edit_cfg.instance_load_folder, f'{v}.pt'))
        trainer = replace_instance(trainer, new_gaussians_dict=gs_dict, **edit_cfg.args, )

    os.makedirs(os.path.join(log_dir, f"videos_edit_{edit_task}"), exist_ok=True)
...
if __name__ == "__main__":
    ...
    # edit task
    parser.add_argument("--edit_cfg", type=str)

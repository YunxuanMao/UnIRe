...
class DrivingDataset(SceneDataset):
    ...
    def get_lidar_samples_t(self, 
        ...
        # randomly sample points
        sampled_idx = self.lidar_source.timesteps == timestep
        sampled_pts = self.lidar_source.pts_xyz[sampled_idx].to(device)
        ...
        # valid_mask = self.check_pts_visibility(sampled_pts)
        # sampled_pts = sampled_pts[valid_mask]
        # sampled_color = sampled_color[valid_mask]
        # sampled_time = sampled_time[valid_mask]
        
        if hasattr(self.lidar_source, 'grounds'):
            return sampled_pts, sampled_color, sampled_time, self.lidar_source.grounds[sampled_idx].to(device)
        
        return sampled_pts, sampled_color, sampled_time, None
    ...
    def project_lidar_pts_on_images(self, delete_out_of_view_points=True):
        ...
        for cam in self.pixel_source.camera_data.values():
            ...
            for frame_idx in tqdm(
                ...
                lidar_points_maps.append(lidar_points)

...
class CameraData(object):
    def __init__(
        ...
        if load_optical_flow:
            self.load_optical_flow()
        else:
            self.optical_flow = None
            self.optical_flow_certainty = None
        ...
        self.lidar_points = None # will be loaded by: self.load_depth()
    ...
    def load_optical_flow(self):
        ...
        optical_flow_file =  os.path.join(self.data_path, "optical_flow", f"optical_flow_{self.cam_id}.pt")
        
        if os.path.exists(optical_flow_file):
            print(f"Load optical flow from {optical_flow_file}...")
            ckpt = torch.load(optical_flow_file)
            # resize them to the load_size
            end_points = ckpt['bwd_warp'][self.start_timestep:self.end_timestep]
            certainty = ckpt['bwd_certainty'][self.start_timestep:self.end_timestep]

            H0, W0 = end_points.shape[1], end_points.shape[2]

            end_points = torch.nn.functional.interpolate(
                end_points.permute(0, 3, 1, 2),
                size=(self.load_size[0], self.load_size[1]),
                mode='bilinear',
                align_corners=True
            ).permute(0, 2, 3, 1)

            end_points[..., 0] = (end_points[..., 0] + 1) / 2 * (self.load_size[1] - 1)
            end_points[..., 1] = (end_points[..., 1] + 1) / 2 * (self.load_size[0] - 1)
            ...
            if self.undistort:
                end_points_list = []
                certainty_list = []
                print("undistorting optical flow")
                for i in range(len(end_points)):
                    end_points_list.append(torch.from_numpy(np.array(cv2.undistort(
                        end_points[i].numpy(),
                        self.intrinsics[0].numpy(),
                        self.distortions[0].numpy(),
                    ))))
                    certainty_list.append(torch.from_numpy(np.array(cv2.undistort(
                        certainty[i].numpy(),
                        self.intrinsics[0].numpy(),
                        self.distortions[0].numpy(),
                    ))))
                end_points = torch.stack(end_points_list)
                certainty = torch.stack(certainty_list)
            y, x = torch.meshgrid([torch.linspace(0, self.load_size[0]-1, self.load_size[0]), torch.linspace(0, self.load_size[1]-1, self.load_size[1])], indexing='ij')
            ij = torch.stack([x, y], -1)
            self.optical_flow = ij - end_points.float()
            self.optical_flow_certainty = certainty.float()
        else:
            print(f"{optical_flow_file} not found!")
            self.optical_flow = None
            self.optical_flow_certainty = None
    ...
    def get_image(self, frame_idx: int) -> Dict[str, Tensor]:
        ...
        egocar_mask = None
        optical_flow, optical_flow_certainty = None, None
        ...
        if self.optical_flow is not None and frame_idx > 0:
            optical_flow = self.optical_flow[frame_idx-1]
            optical_flow_certainty = self.optical_flow_certainty[frame_idx-1]
            if self.downscale_factor != 1.0:
                optical_flow = torch.nn.functional.interpolate(
                    optical_flow.unsqueeze(0).permute(0, 3, 1, 2),
                    scale_factor=self.downscale_factor,
                    mode='bilinear',
                    align_corners=True
                ).squeeze(0).permute(1, 2, 0) * self.downscale_factor
                optical_flow_certainty = torch.nn.functional.interpolate(
                    optical_flow_certainty.unsqueeze(0).unsqueeze(0),
                    scale_factor=self.downscale_factor,
                    mode='bilinear',
                    align_corners=True
        ...
        if self.lidar_depth_maps is not None:
            ...
            lidar_points = self.lidar_points[frame_idx].to(self.device)
            if self.downscale_factor != 1.0:
        ...
        _image_infos = {
            ...
            "optical_flow_certainty": optical_flow_certainty
        }
        ...
        cam_infos = {
            ...
            "camera_to_world": c2w,
            "camera_to_world_prev": c2w_prev,
            "camera_to_world_next": c2w_next,
...
class ScenePixelSource(abc.ABC):
    ...
    def prepare_novel_view_render_data(self, dataset_type: str, traj: torch.Tensor) -> list:
        ...
        for i in range(len(traj)):
            ...
            camera_to_world_prev = None
            if i > 0:
                camera_to_world_prev = traj[i-1].to(self.device)
            cam_infos = {
                "camera_to_world": c2w,
                "camera_to_world_prev": camera_to_world_prev,
                "intrinsics": intrinsics,

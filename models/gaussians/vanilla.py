...
class VanillaGaussians(nn.Module):
    ...
    def create_from_pcd(self, init_means: torch.Tensor, init_colors: torch.Tensor) -> None:
        ...
        # quats = torch.zeros((init_means.shape[0], 4)).to(self.device)
        # quats[:,0] = 1
    ...
    def add_gaussians(self, optimizer, gaussians) -> Tuple:
        """
        This function add gaussians
        """
        new_means = gaussians["_means"]
        new_feature_dc = gaussians["_features_dc"]
        new_feature_rest = gaussians["_features_rest"]
        new_opacities = gaussians["_opacities"]
        new_scales = gaussians["_scales"]
        new_quats = gaussians["_quats"]
        num_new = len(new_means)

        self._means = Parameter(torch.cat([self._means.detach(), new_means], dim=0))
        # self.colors_all = Parameter(torch.cat([self.colors_all.detach(), split_colors, dup_colors], dim=0))
        self._features_dc = Parameter(torch.cat([self._features_dc.detach(), new_feature_dc], dim=0))
        self._features_rest = Parameter(torch.cat([self._features_rest.detach(), new_feature_rest], dim=0))
        ...
        param_groups = self.get_gaussian_param_groups()
        add_in_optim(optimizer, num_new, param_groups)
        

    def get_gaussians(self, cam: dataclass_camera, *args) -> Dict:
    ...
    def export_gaussians_to_ply(self, alpha_thresh: float = -0.1) -> Dict:
        means = self._means

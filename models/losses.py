...
def smooth_loss(images, flows, edge_constant=150, mask=None):
    '''
    images: [h,w,3]
    flows: [h,w,3]
    mask: [h,w]
    ...
    flows = flows.permute(2,0,1).unsqueeze(0)

    img_gx, img_gy = image_grads(images, stride=2)

    # abs_fn = lambda x: x ** 2

    weights_x = torch.exp(-torch.abs(edge_constant * img_gx).mean(dim=1, keepdim=True))
    weights_y = torch.exp(-torch.abs(edge_constant * img_gy).mean(dim=1, keepdim=True))
    if mask is not None:
        weights_x = weights_x * mask[2:-2]
        weights_y = weights_y * mask[:, 2:-2]

    # Compute second derivatives of the predicted smoothness.
    flow_gx, flow_gy = image_grads(flows)
    flow_gxx, unused_flow_gxy = image_grads(flow_gx)
    unused_flow_gyx, flow_gyy = image_grads(flow_gy)

    # Compute weighted smoothness
    return ((weights_x * torch.abs(flow_gxx)).mean() + (weights_y * torch.abs(flow_gyy)).mean()) / 2.0
...
from utils.flow_viz import map_colors
# from utils.flow_viz import flow_to_color_2d
...
def render(
    ...
    with torch.no_grad():
        ...
        for i in tqdm(indices, desc=f"rendering {dataset.split}", dynamic_ncols=True):
            ...
            green_background = torch.tensor([255, 255, 255.]) / 255.0
            green_background = green_background.to(rgb.device)
            if 'instance' in results:
                instance = results['instance']
                instances.append(get_numpy(instance))
            if 'flow' in results and results['flow'].shape[-1] == 2:
                # flow_color = flow_to_color_2d(get_numpy(results['flow']))
                flow_color = flow_vis_torch.flow_to_color(results['flow'].permute(2, 0, 1)).permute(1, 2, 0)
                flow_colors.append(get_numpy(flow_color) / 255.0)
            ...
            if compute_metrics:
                ...
                if "lidar_depth_map" in image_infos:
                    depth_map = image_infos["lidar_depth_map"].squeeze()
                    pred_depth = results["depth"].squeeze()
                    mask = (depth_map > 0)
                    depth_l1.append(F.l1_loss(depth_map[mask], pred_depth[mask]).double().item())
    ...
    results_dict["depth_l1"] = non_zero_mean(depth_l1) if compute_metrics else -1
    ...
    if len(flow_colors) > 0:
        results_dict["flow"] = flow_colors
    if len(instances) > 0:
        results_dict["instance"] = instances
    return results_dict
    ...
    for key in keys:
        ...
        for i in range(num_timestamps):
            ...
            if key == "gt_sky_masks":
                ...
                h, w, num_instance = frames[0].shape
                for idx in range(len(frames)):
                    frames[idx] = np.argmax(frames[idx], axis=-1)
                if save_images:
                    if i == 0:
                        os.makedirs(tmp_save_pth.replace(".mp4", ""), exist_ok=True)
                    for j, frame in enumerate(frames):
                        np.save(tmp_save_pth.replace(".mp4", f"/{i:03d}_{j:03d}.npy"), frame)
                for idx in range(len(frames)):
                    frames[idx] = map_colors(frames[idx].reshape(-1, 1), min_value=0, max_value=num_instance).reshape(h, w, 3)
